...
Include the following properties in your `pom.xml` file to track the version of {SpringBoot} and the Vert.x Maven Plugin you are using:
...
<project>
  ...
  <properties>
    <vertx.version>{version-spring-boot}</vertx.version>
    <vertx-maven-plugin.version>{version-spring-boot-maven-plugin}</vertx-maven-plugin.version>
  </properties>
  ...
</project>
----
