...
. Edit the `launcher_cr.yaml` custom resource to add your OpenShift Console URL:
+
.launcher_cr.yaml
...
. Install the `launcher_cr.yaml` custom resource to deploy the {name-launcher} tool.
+
[source,bash,options="nowrap",subs="attributes+"]
----
