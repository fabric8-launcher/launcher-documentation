...
To configure your {OpenShiftLocal} to automatically trigger a build when a new commit is made to GitHub, we will use the Ultrahook's proxy cloud application.
...
With your {OpenShiftLocal}, that webhook points to a local IP address, which can't work.
