...
----
$ oc get pods -w
my-database-1-UID   1/1       Running   0         45s
my-database-1-deploy   0/1       Completed   0         53s
----
+
The `my-database-1-UID` pod should have a status of `Running` and should be indicated as ready once it is fully deployed and started.
...
----
+
Your `{app-name}-1-UID` pod should have a status of `Running` and should be indicated as ready once it is fully deployed and started.
