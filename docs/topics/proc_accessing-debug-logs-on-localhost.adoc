...
+
For example, when debug logging in the xref:mission-rest-http-nodejs[{name-example-http-api} booster] is set to log the `name` variable in the `/api/greeting` method:
+
[source,bash,options="nowrap",subs="attributes+"]
----
