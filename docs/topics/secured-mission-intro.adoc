...
* All OpenShift configuration items needed to stand up a preconfigured {RHSSO} server
* A default SSO server configuration 
* Preconfigured users and security settings
...
a|
Allows the end user to deploy the {RHSSO} module without needing to know the internals or how to configure {RHSSO}.

a|
In production, do not store private keys under version control. They should be added by the server administrator.
...
a|
The boosters are simplified and do not require certificates to be generated for each runtime.

a|
In production, a secure application should use HTTPS rather than plain HTTP.

a|
**The token timeout has been increased from the default of 1 minute to 10 minutes.**

a|
Provides a better user experience when working with the command line examples. From a security perspective, this configuration extends the time an attacker would have to guess the access token.

a|
In production, it is recommended to keep this window short as it makes it much harder for a potential attacker to guess the current token.
