...
[id='deploying-the-secured-example-application-using-the-oc-client_{context}']
= Deploying the {name-example-secured} example application using the `oc` CLI client

.Prerequisites

* The example application created using the {name-launcher} tool on a {OpenShiftLocal}.
ifndef::parameter-ocp[For more information, see xref:creating-the-secured-example-application-using-launcher_{context}[].]
