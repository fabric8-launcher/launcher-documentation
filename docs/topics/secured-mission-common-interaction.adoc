...
. The services responds with an `HTTP 401 Unauthenticated` status code.
...
. Click on _Invoke_. 
. Confirm the service responds with an `HTTP 403 Unauthorized` status code since the _admin_ user is not authorized to access the Greeting service.