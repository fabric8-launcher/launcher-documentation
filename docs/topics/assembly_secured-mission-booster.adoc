...
= {name-example-secured} mission - {runtime} booster
...
The {name-example-secured} booster secures a REST endpoint using link:https://access.redhat.com/products/red-hat-single-sign-on[{RHSSO}^]. (This booster expands on the {name-example-http-api} booster).
