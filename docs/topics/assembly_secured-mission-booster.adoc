...
= {name-example-secured} example for {runtime}
...
The {name-example-secured} example secures a REST endpoint using link:https://access.redhat.com/products/red-hat-single-sign-on[{RHSSO}^]. (This example expands on the {name-example-http-api} example).
