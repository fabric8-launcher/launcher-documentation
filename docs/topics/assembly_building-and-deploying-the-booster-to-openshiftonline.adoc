...
You have two options for executing the {parameter-mission-name} booster on {OpenShiftOnline}. Although both methods use the same `oc` commands to deploy your application, using {launcher-oso} provides an automated booster deployment workflow that executes the `oc` commands for you.
