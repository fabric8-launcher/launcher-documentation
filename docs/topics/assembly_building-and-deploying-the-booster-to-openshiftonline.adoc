...
You have two options for executing the {parameter-mission-name} booster on {OpenShiftOnline}:

* xref:deploying-the-booster-using-launcher-oso_{context}[Using {launcher-oso}]
* xref:deploying-the-{parameter-mission}-booster-using-the-oc-cli-client_{context}[Using the `oc` CLI client]

Although both methods use the same `oc` commands to deploy your application, using {launcher-oso} provides an automated booster deployment workflow that executes the `oc` commands for you.
