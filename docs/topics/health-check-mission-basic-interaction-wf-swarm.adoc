...
Once you have the {WildFlySwarm} booster deployed, you will have a service called  `{app-name}` running that exposes a two REST endpoints: `/api/greeting` which returns a name as a String and `/api/stop`, which forces the service to become unresponsive as means to simulate a failure.
...
. To simulate a service internal failure and to trigger the OpenShift selfhealing capabilities, invoke the `/api/stop` endpoint and verify the availability of the `/api/greeting` endpoint shortly after that. It should return a HTTP status 503:
+
[source,bash,option="nowrap",subs="attributes+"]
----
$ curl http://{app-name}-{project-name}.{osl-route-hostname}/api/stop
