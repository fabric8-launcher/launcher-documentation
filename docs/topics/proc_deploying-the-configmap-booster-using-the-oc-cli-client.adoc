...
----
+
The `{app-name}-1-UID` pod should have a status of `Running` once its fully deployed and started.
ifdef::configmap-vertx[]
You should also wait for your pod to be _ready_ before proceeding, which is shown in the `READY` column. For example, `{app-name}-1-UID` is _ready_ when the `READY` column is `1/1`.
