...
<2> These are properties for setting the {VertX} and {VertX} Maven plugin version to use as well as the name of the {VertX} verticle.
...
<8> link:https://wiki.eclipse.org/M2E_compatible_maven_plugins[Lifecycle metadata] that works with the {VertX} Fabric8 Maven Plugin for building and deploy the application.
