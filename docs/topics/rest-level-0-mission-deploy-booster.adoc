...
. Get your authentication token for using the `oc` CLI client with {OpenShiftLocal}. You can find this in the _Command Line Tools_ section your {OpenShiftLocal} Web console.

. Authenticate your `oc` CLI client with {OpenShiftLocal} by using your authentication token.
+
...
----
+
These commands install any missing package dependencies, then using the Nodeshift package, launch the S2I process on {OpenShiftLocal} to start the pod.
endif::http-api-nodejs[]
