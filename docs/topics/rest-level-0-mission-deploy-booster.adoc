...
include::booster-deploy-launchpad-openshift-local.adoc[leveloffset=+1]

[[deploy-booster-osl-cli]]
=== Deploy Using the `oc` CLI Client

. Get your authentication token for using the `oc` CLI client with your {OpenShiftLocal} account. You can find this in your {OpenShiftLocal} Web console. In the above example, your token would be located at `https://192.168.99.100:8443/console/command-line`. 
