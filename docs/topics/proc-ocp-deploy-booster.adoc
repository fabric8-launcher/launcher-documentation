...
.Prerequisites
--
* Have a booster created using the {launcher} application in xref:oso-create-booster[{OpenShiftOnline}] or xref:osl-create-booster[a {OpenShiftLocal}].
* Have access to an {OpenShiftContainerPlatform} Web console.
* Have the `oc` CLI client installed.
--

.Procedure
. Clone your booster's Github repository, or download and extract the booster's ZIP.
