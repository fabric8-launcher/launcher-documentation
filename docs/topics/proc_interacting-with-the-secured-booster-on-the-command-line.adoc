...
[#interacting-with-the-secured-booster-on-the-command-line]
= Interacting with the {name-mission-secured} Booster on the Command Line

Request a token by sending a HTTP POST request to the {RHSSO} server.
In the following example, the the link:[jq CLI tool] is used to extract the token value from the JSON response.
