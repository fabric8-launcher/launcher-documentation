...
----
+
For example, when using the link:https://expressjs.com/en/starter/generator.html[express generator] to create your application, the entry point is set to `./bin/www` by default. Some of the examples, such as the xref:mission-rest-http-nodejs[{name-example-http-api} booster], use `./bin/www` as an entry point.
