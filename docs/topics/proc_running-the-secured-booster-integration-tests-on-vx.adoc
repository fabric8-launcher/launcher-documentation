...
If you deployed an SSO server beforehand, e.g. by executing `oc create -f service.sso.yaml`, set the system property `skip.sso.init` to `true` when running the tests (example shown below).
The tests will then use the existing SSO server and won't deploy their own, nor would they destroy the existing one.
