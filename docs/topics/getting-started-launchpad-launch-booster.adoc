...
NOTE: In the GitHub repository, a webhook is created under _Settings_ -> _Webhooks_. When launching boosters with {OpenShiftLocal}, this link points to {OpenShiftLocal} and will not be accessible from outside the host {OpenShiftLocal} is running on.
