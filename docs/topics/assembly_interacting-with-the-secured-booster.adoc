...
The Secured booster provides a default HTTP endpoint that accepts `GET` requests if the caller is authenticated and authorized.
...
The `sso` directory in the ZIP archive with the booster contains a Java client you can use to interact with the {name-mission-secured} booster.
