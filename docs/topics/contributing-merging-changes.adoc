...
. Test if all the books build correctly with your changes by executing the `$REPO_HOME/scripts/buildGuides.sh` script. If the output contains no errors, the guides build correctly.
...
. Execute `$REPO_HOME/scripts/buildGuides.sh` to test whether the guides build correctly with the changes introduced by your merge.
