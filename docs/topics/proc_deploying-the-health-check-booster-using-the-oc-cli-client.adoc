...
----
+
The `{app-name}-1-UID` pod should have a status of `Running` once its fully deployed and started. You should also wait for your pod to be ready before proceeding, which is shown in the `READY` column. For example, `{app-name}-1-UID` is ready when the `READY` column is `1/1`.
