...
+
For example, if you had debug logging in the xref:mission-rest-http-nodejs[{name-example-http-api} booster] to log the `name` variable in the `/api/greeting` method:
+
[source,bash,options="nowrap",subs="attributes+"]
----
$ oc get routes
