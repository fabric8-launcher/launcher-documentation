...
* Create the booster application using {parameter-deployment}.
...
* Authenticate the `oc` client . For more information, see xref:authenticating-the-oc-cli-client_{context}[].
...
----
+
These commands install any missing package dependencies, then using the xref:about-nodeshift[Nodeshift] package, launch the link:{link-s2i-process}[S2I process] on OpenShift to start the pod.
endif::http-api-nodejs[]
...
----
+
Your `{app-name}-1-aaaaa` pod should have a status of `Running` once it's fully deployed and started.
