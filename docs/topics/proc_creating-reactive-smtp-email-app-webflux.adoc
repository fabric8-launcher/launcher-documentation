[id='creating-an-http-smtp-mail-service-with-webflux']
= Creating a reactive {SpringBoot} SMTP mail app with WebFlux
...
. Add `vertx-spring-boot-starter` and `vertx-spring-boot-starter-mail` as dependencies in the `pom.xml` file of your project.
...
[source,bash,options="nowrap",subs="attributes+"]
----
$ java -jar target/vertx-spring-boot-sample-mail-0.0.1-SNAPSHOT.jar 
----
