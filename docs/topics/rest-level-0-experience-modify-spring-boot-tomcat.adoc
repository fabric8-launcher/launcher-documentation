...
public void test_modified_service_invocation() {
  ...
  String response  = template.getForObject(appURL+"/api/supportedLanguages",String.class);
  ...
  WorldGreeting wgResponse = template.postForEntity(appURL+"/api/saveWorldGreeting", wgRequest, WorldGreeting.class).getBody();
  Assert.assertTrue(wgRequest.equals(wgResponse));
            
  response  = template.getForObject(appURL+"/api/supportedLanguages",String.class);
