
[id='deploying-the-secured-booster-to-openshiftlocal_{context}']
[id='deploying-the-secured-example-application-to-openshiftlocal_{context}']
= Deploying the {name-example-secured} example application to {OpenShiftLocal}

:parent-context: {context}
:context: {parent-context}-osl

include::proc_getting-the-launcher-tool-url-and-credentials.adoc[leveloffset=+1]

include::proc_creating-the-secured-example-application-using-launcher.adoc[leveloffset=+1]

:parameter-deployment: {OpenShiftLocal}
:parameter-openshiftlocal:
include::proc_authenticating-the-oc-cli-client.adoc[leveloffset=+1]
:parameter-openshiftlocal!:

include::proc_deploying-the-secured-example-application-using-the-oc-cli-client.adoc[leveloffset=+1]
:parameter-deployment!:

:context: {parent-context}
