...
.Prerequisites
* An application you want to debug. For example, the xref:mission-rest-http-spring-boot[{name-example-http-api} example].
...
----
+
For example, if you wanted to add logging to the `GreetingEndpoint` class in the xref:mission-rest-http-spring-boot[{name-example-http-api} example], you would use `GreetingEndpoint.class`.
