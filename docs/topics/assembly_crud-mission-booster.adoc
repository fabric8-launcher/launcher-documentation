...
The user can also use an UI provided by the example to use the application.
...
This example also contains a set of automated xref:running-the-example-integration-tests_{context}[integration tests] that can be used to verify that the application is fully integrated with the database.
