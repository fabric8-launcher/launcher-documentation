...
.Prerequisites 
* An application you want to debug. For example, the xref:mission-http-api-spring-boot[{name-mission-http-api} booster].
...
----
+
For example, if you wanted to add logging to the `GreetingEndpoint` class in the xref:mission-http-api-spring-boot[{name-mission-http-api} booster], you would use `GreetingEndpoint.class`.
