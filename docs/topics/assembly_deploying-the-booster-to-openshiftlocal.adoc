...
[id='deploying-the-example-to-openshiftlocal_{context}']
= Deploying the {parameter-example-name} example to {OpenShiftLocal}

:parent-context: {context}
:context: {parent-context}-osl

Use one of the following options to execute the {parameter-example-name} example locally on {OpenShiftLocal}:

* xref:deploying-the-booster-using-the-launcher-tool_{context}[Using {name-launcher}]
* xref:deploying-the-{parameter-example}-booster-using-the-oc-cli-client_{context}[Using the `oc` CLI client]
