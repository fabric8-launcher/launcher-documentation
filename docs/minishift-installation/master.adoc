include::topics/templates/document-attributes.adoc[]

:install-launcher-osl:
//var for front-end topics, if below is defined in topic, its used in docs, if not its used in the front end
:docs-topic:

= {name-guide-minishift-installation}

:context: minishift

[.lead]
This guide walks you through the process of installing the {name-launcher} tool on an {OpenShiftLocal}.
This includes link:https://www.openshift.org/minishift/[{Minishift}], an all-in-one VM with link:https://www.okd.io[OKD^], or link:https://developers.redhat.com/products/cdk/overview/[{CDK}], a VM that includes OpenShift Container Platform.

include::topics/con_how-the-launcher-tool-works.adoc[leveloffset=+1]

//TODO: link to this chapter from the paragraph about using Minishft for local development
include::topics/proc_installing-a-openshiftlocal.adoc[leveloffset=+1]

//TODO: revise this chapter, should probably be removed
include::topics/proc_starting-and-configuring-the-openshiftlocal-for-the-launcher-tool.adoc[leveloffset=+1]

//TODO: replace with setting up OAuth
include::topics/proc_creating-a-github-personal-access-token.adoc[leveloffset=+1]

[id='installing-launcher-tool_{context}']
== Installing {name-launcher} Tool
Install a local customized instance of the {name-launcher} tool, which allows you to test the functionality or make modifications to the service using a web interface.

//TODO: Replaced with operator-based installation procedure, as described in RHOARDOC-1603
include::topics/proc_installing-launcher-tool-addon.adoc[leveloffset=+2]

//TODO: Remove this section.
include::topics/proc_installing-launcher-tool-manually.adoc[leveloffset=+2]

// temporary location for Nexus install
[appendix]
include::topics/proc_using-a-nexus-repository-server-on-a-openshiftlocal.adoc[leveloffset=+1]

[appendix]
include::topics/ref_glossary.adoc[leveloffset=+1]
