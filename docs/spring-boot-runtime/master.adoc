...
{SpringBoot} has a version of its runtime that is tested and verified to run on OpenShift. Some components, like the Embedded Tomcat Container and Java container image, when used with {SpringBoot} are provided as part of a Red Hat subscription.
