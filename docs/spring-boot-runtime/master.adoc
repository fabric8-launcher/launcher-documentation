...
The {SpringBoot} runtime version `{SpringBootVersion}` is tested and verified to run with the Embedded Apache Tomcat Container on OpenShift. This embedded container, as well as other components such as the Java container image, is provided as part of a Red Hat subscription when used with {SpringBoot}.
