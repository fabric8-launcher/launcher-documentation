...
Debug logging is a way to add granular logging to an application can be explicitly enabled only when performing debugging. This allows you to keep the logging capability in your application and only enable it when needed.

include::topics/proc_add-debug-logging-nodejs.adoc[leveloffset=+3]

include::topics/proc_show-debug-logging-local-nodejs.adoc[leveloffset=+3]

include::topics/proc_show-debug-logging-openshift-nodejs.adoc[leveloffset=+3]
